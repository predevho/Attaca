...
* 단계: BE(인증/프로필/파일/DB + VERIFIED-PERFORMER + FEED) + FE(인증/프로필/카카오) 완료. 다음은 FE 인증연주자/피드 화면 또는 다음 도메인(PERFORMANCE/RECRUITMENT/CHAT — 각 도메인 문서 선작성 필요).
* 확정된 기술 스택
...
* 도메인 문서 없이 해당 도메인 구현 금지. 현재 구현된 도메인: COMMON, MEMBER, VERIFIED-PERFORMER(2026-07-16 BE), FEED(2026-07-17 BE). 미착수(문서도 없음): PERFORMANCE, RECRUITMENT, CHAT.
